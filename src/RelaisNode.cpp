...
RelaisNode::RelaisNode(uint16_t defaults, uint16_t invert, uint16_t inputmask) :
		HomieNode("Relais", "Relais (Generic IO)", "io16", true, 1, 16),
...
void RelaisNode::setup() {
#ifndef PCF8575 	// no begin needed for PCF8575
	pinMode(16, OUTPUT);  // Reset on PIN 16
	digitalWrite(16, LOW);
	delay(1);
	...
	advertise("inout").setDatatype("boolean").setRetained(true).setName("IO pin").settable();
	updateRelais(0x0000); // write to GPIO only
}
	...
	for (uint_fast8_t i = 0; i < 16; i++) {
		if ((diff & (1 << i)) != 0) {
			...
			LN.logf("RN::loop", LoggerNode::INFO,
					"Input %d changed to %c%s, new: %x, old: %x, diff: %x",
...
void RelaisNode::loop() {
	...
	if ((++count % 64)==0) readInputs(); // read I2C on every 64th cycle only
}
...
bool RelaisNode::handleInput(const HomieRange& range, const String  &property, const String &value) {
	...
	if (selected_bit & input_mask ) {
		LN.log("RelaisNode::handleInput()", LoggerNode::WARNING, "Tried to set INPUT - ignoring command");
		return true; // return true anyhow, because command was handled
	}
	if (on ^ inverted ) {
...
void RelaisNode::updateRelais(uint16_t updateMask) {
	...
	for (uint_fast8_t i = 0; i < 16; i++) {
		...
		if (changed || ((updateMask & (1 << i)) != 0)) {
			bool inverted = (invert_bitset & (1<< i)) != 0;
			String property("inout");
			HomieInternals::SendingPromise& p = setProperty(property).setRange(i+1);
			p.send(on ^ inverted ? "ON" : "OFF");
			p.resetRange();

		}
	}
	uint16_t readback = io.readGPIOAB();
	uint16_t difference = (relais_bitset ^ readback) & ~input_mask;
	LN.logf("UpdateRelais READBACK", difference ? LoggerNode::ERROR : LoggerNode::DEBUG,
			"Readback %cOK:\n\tW: %x\n\tR: %x\n\tDiff: %x\n\tInputs: %x (Mask: %x)\n",
			difference ? 'N' : ' ', relais_bitset, readback, difference, input_data, input_mask);
	last = relais_bitset;
}
